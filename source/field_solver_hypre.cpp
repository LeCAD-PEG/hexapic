...
void hypre_init_geo() {
	if (grid.boundaries[0].name=="Periodic") hypre.periodic[0] = grid.Nxg - 1;
	else hypre.periodic[0] = 0;
	if (grid.boundaries[2].name=="Periodic") hypre.periodic[1] = grid.Nyg - 1;
	else hypre.periodic[1] = 0;
	...
	if (bcs[0].j1 == hypre.ilower[0]) {
		if (bcs[0].name == "Equipotential") hypre.bcs[0] = 1;
		else if (bcs[0].name == "Dielectric") hypre.bcs[0] = 2;
	}

	if (bcs[1].j1 == hypre.iupper[0]) {
		if (bcs[1].name == "Equipotential") hypre.bcs[1] = 1;
		else if (bcs[1].name == "Dielectric") hypre.bcs[1] = 2;
	}

	if (bcs[2].k1 == hypre.ilower[1]) {
		if (bcs[2].name == "Equipotential") hypre.bcs[2] = 1;
		else if (bcs[2].name == "Dielectric") hypre.bcs[2] = 2;
	}

	if (bcs[3].k1 == hypre.iupper[1]) {
		if (bcs[3].name == "Equipotential") hypre.bcs[3] = 1;
		else if (bcs[3].name == "Dielectric") hypre.bcs[3] = 2;
	}
}
...
void hypre_init_ghost() {
    ...
    for (int r : grid.neighbours.all) {
        if (hypre.size_r[r] > 0) {
            MPI_Request req;
            MPI_Recv_init(hypre.recv_buff[r].data(), hypre.size_r[r],
                          MPI_DOUBLE, r, HYPRE_TAG_V, comm, &req);
    ...
    for (int r : grid.neighbours.all) {
        if (hypre.size_s[r] > 0) {
            MPI_Request req;
            MPI_Send_init(hypre.send_buff[r].data(), hypre.size_s[r],
                          MPI_DOUBLE, r, HYPRE_TAG_V, comm, &req);
            hypre_persist_v_reqs.push_back(req);
        }
    }

    // For rho exchange (opposite buffers): Recv into send_buff (size_s), Send from recv_buff (size_r)
    for (int r : grid.neighbours.all) {
        if (hypre.size_s[r] > 0) {
            MPI_Request req;
            MPI_Recv_init(hypre.send_buff[r].data(), hypre.size_s[r],
                          MPI_DOUBLE, r, HYPRE_TAG_RHO, comm, &req);
    ...
    for (int r : grid.neighbours.all) {
        if (hypre.size_r[r] > 0) {
            MPI_Request req;
            MPI_Send_init(hypre.recv_buff[r].data(), hypre.size_r[r],
                          MPI_DOUBLE, r, HYPRE_TAG_RHO, comm, &req);
            hypre_persist_rho_reqs.push_back(req);
        }
    }
...
void hypre_init_solver() {
    ...
    for (int j = hypre.ilower[1]; j <= hypre.iupper[1]; j++) {
        for (int i = hypre.ilower[0]; i <= hypre.iupper[0]; i++) {
            ...
            if ((i == bcs[0].j1 && i == bcs[0].j2)) { // xmin boundary
            	...
            	else if (bcs[0].name == "Dielectric") bcs_ij[0] = 2;
            }
            ...
            if (i == bcs[1].j1 && i == bcs[1].j2) { // xmax boundary
            	...
            	else if (bcs[1].name == "Dielectric") bcs_ij[1] = 2;
            }
            ...
            if (j == bcs[2].k1 && j == bcs[2].k2) { // ymin boundary
            	...
            	else if (bcs[2].name == "Dielectric") bcs_ij[2] = 2;
            }
            ...
            if (j == bcs[3].k1 && j == bcs[3].k2) { // ymax boundary
            	...
            	else if (bcs[3].name == "Dielectric") bcs_ij[3] = 2;
            }

            // if any dirichlet -> fixed boundary!
            if (d) values[0] = dx2_inv;
            else {
            	if (bcs_ij[0] != 2) {
            		values[0] -= dx2_inv;
            		values[1] += dx2_inv;
            ...
            HYPRE_StructMatrixSetValues(A_h, index, nentries,
    ...
    HYPRE_StructPFMGSetRelaxType(pfmg, 2);  // Red-Black Gauss-Siedel, often better than Jacobi
...
void hypre_finish_V_halo() {
  if (!hypre_persist_v_reqs.empty())
    MPI_Waitall((int)hypre_persist_v_reqs.size(), hypre_persist_v_reqs.data(), MPI_STATUSES_IGNORE);
...
void hypre_post_rho_halo() {
  ...
  if (!hypre_persist_rho_reqs.empty())
    MPI_Startall((int)hypre_persist_rho_reqs.size(), hypre_persist_rho_reqs.data());
}

void hypre_finish_rho_halo() {
  if (!hypre_persist_rho_reqs.empty())
    MPI_Waitall((int)hypre_persist_rho_reqs.size(), hypre_persist_rho_reqs.data(), MPI_STATUSES_IGNORE);
...
//     if (!hypre_persist_v_reqs.empty()) {
//         MPI_Startall(static_cast<int>(hypre_persist_v_reqs.size()), hypre_persist_v_reqs.data());
//         MPI_Waitall(static_cast<int>(hypre_persist_v_reqs.size()), hypre_persist_v_reqs.data(), MPI_STATUSES_IGNORE);
//     }
...
void hypre_field_solver() {
	HYPRE_StructPFMGSolve(pfmg, A_h, b_h, x_h);
}
...
//     if (!hypre_persist_rho_reqs.empty()) {
//         MPI_Startall(static_cast<int>(hypre_persist_rho_reqs.size()), hypre_persist_rho_reqs.data());
//         MPI_Waitall(static_cast<int>(hypre_persist_rho_reqs.size()), hypre_persist_rho_reqs.data(), MPI_STATUSES_IGNORE);
//     }
...
void hypre_cleanup() {
    ...
    for (auto &req : hypre_persist_v_reqs) { MPI_Request_free(&req); }
    for (auto &req : hypre_persist_rho_reqs) { MPI_Request_free(&req); }
    hypre_persist_v_reqs.clear();
    hypre_persist_rho_reqs.clear();
