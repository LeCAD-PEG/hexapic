 ...
 * 
 * @file hexapic.hpp
 * @brief Core data structures, types, and global parameters for the HEXAPIC Particle-in-Cell (PIC) code.
 */
...
struct Boundary {
	...
	void add_parameters(std::string NAME, int J1,
              int J2, int K1,
              int K2) {
...
EXTERNAL void particles_init(), inject_particle(int, REAL, REAL),
              ...
              particle_mover_boris(const std::vector<Cell*>&),
              save_output(), num_param_init(), create_output(),
              grid2part(REAL, REAL, double*, REAL *),
              particle_boundaries(), read_input_file(char **),
              initial_particle_load(), inject_particles(), plot_profiling(),
              decompose_domain(), send_receive_particles(),
              add_received_particles(), cells_init(),
              collisions_init(), MCC(), remove_particle(int, int, int),
              particle_wall_interaction(size_t, int, int, int), PSI_init(),
              source_init(), source(), source_heating(), hypre_init(),
		      hypre_cleanup(), hypre_source_update(), hypre_field_solver(),
		      hypre_get_V(), hypre_post_V_halo(), hypre_finish_V_halo(),
		      hypre_post_rho_halo(), hypre_finish_rho_halo(), hypre_rho_update(),
		      hypre_update_Vlocal(); 
