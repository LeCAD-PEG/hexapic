...
class Simulation(PICMI_Simulation):
    def write_input_file(self, file_name="input_file.inp", description=""):
        ...
        BoundarycX = [
            grid.lower_boundary_conditions[0],
            grid.upper_boundary_conditions[0],
        ]
        BoundarycY = [
            grid.lower_boundary_conditions[1],
            grid.upper_boundary_conditions[1],
        ]

        if "periodic" in BoundarycX:
            assert not ("dirichlet" in BoundarycX), Exception(
                "For PBC bc_xmin and bc_xmax should be 'periodic'"
            )
            ...
            if self.applied_fields:
                ...
                assert field.Ex == None, Exception(
                    "For PBC in x direction Ex should be 'None'"
                )
            content += "\t PeriodicFlagX1 = 1 \n"
            PeriodicFlagX1 = True
        if "periodic" in BoundarycY:
            assert not ("dirichlet" in BoundarycY), Exception(
                "For PBC bc_ymin and bc_ymax should be 'periodic'"
            )
            grid.lower_boundary_conditions[1] = "periodic"
            grid.upper_boundary_conditions[1] = "periodic"
            if self.applied_fields:
                field = self.applied_fields[0]
                assert field.Ey == None, Exception(
                    "For PBC in y direction Ey should be 'None'"
                )
        ...
        if ("periodic" in BoundarycX) or ("periodic" in BoundarycY):
            content += "\t FieldSolverFlag = 5 \n"
            content += "\t ParticleDiagnosticFlag = 0 \n}\n"
            # Periodic DADI field solver for XOOPIC
        else:
            content += "\t FieldSolverFlag = 1 \n"
            content += "\t ParticleDiagnosticFlag = 0 \n}\n"
        ...
        for i, specie in enumerate(self.species):
            ...
            if inj is not None:
                ...
                inj_dx = (
                    grid.upper_bound[0] - grid.lower_bound[0]
                ) / grid.number_of_cells[0]
                inj_dy = (
                    grid.upper_bound[1] - grid.lower_bound[1]
...
MCC = [
    ["Elastic", "electron", "H", "1", "eH_el.txt"],
...
source0 = {
    "n_inj": 0.0,  # number of injected particles [s^-1 m^-3]
...
for i, specie in enumerate(particle_names):
    ...
    for j in range(3):
        velth[specie][j] = sqrt(
            specie_temperature[i] * constants.q_e / particle_mass[specie]
        )
    veld[specie] = [0.0, 0.0, 0.0]


grid = PICMI_Cartesian2DGrid(
...
solver = PICMI_ElectrostaticSolver(
    grid,
...
for i, p in enumerate(particle_names):
    init_dist = PICMI_UniformDistribution(
        specie_density[i],
        lower_bound=[grid.lower_bound[0], grid.lower_bound[1], None],
        upper_bound=[grid.upper_bound[0], grid.upper_bound[1], None],
        rms_velocity=velth[p],
        directed_velocity=veld[p],
        fill_in=None,
    )
    particle_layout = PICMI_PseudoRandomLayout(
        n_macroparticles=specie_mp[i],
        n_macroparticles_per_cell=None,
        seed=None,
        grid=None,
    )
    specie = Species(
        ...
        name=p,
        charge_state=None,
        charge=particle_charge[p],
        mass=particle_mass[p],
        initial_distribution=init_dist,
        particle_shape=None,
        density_scale=None,
        method="Boris",
    )
    inject_dist = PICMI_UniformFluxDistribution(
        flux=particle_currents[i],
    ...
    sim.add_species(species=specie, layout=particle_layout, initialize_self_field=None)
