...
_**Threshold framework:** All thresholds are adaptive, computed as $
	ext{threshold} = K \times (\text{physics\_err} + \text{float\_err})$
where $K=10$ is a safety factor. Physics error captures Boris integrator $O(\Delta t^2)$ phase drift, and float error captures $O(\epsilon_m \sqrt{N})$ accumulation over $N$ output steps ($\epsilon_m \approx 2.22 \times 10^{-16}$)._
...
- **Metrics & Pass Criteria:**
    ...
    1. **Energy Conservation (Absolute):**
        $\epsilon_E = \max|E_k(t)-E_k(0)|$, $E_k = \frac{1}{2}m(v_x^2+v_y^2+v_z^2)$

        **Pass if:** $\epsilon_E < K \cdot \epsilon_m \cdot \sqrt{N}$

    2. **4-Radius Separation:**
        $\Delta_{\text{sep}} = \max_t |\mathbf{x}_{e^-}(t) - \mathbf{x}_{e^+}(t)|$
        expected $4 R_L$ where $R_L = \frac{m v_\perp}{|q| B_0}$

        **Pass if:** $|\Delta_{\text{sep}} - 4 R_L| < K \cdot (4 R_L \cdot \frac{20(\omega_c \Delta t)^2}{12} + 4 R_L \cdot \epsilon_m \sqrt{N})$

    3. **Y-Projection Trajectory:**
        $\text{Extent} = (\max(y) - \min(y)) \cdot \Delta y$
        expected $2 R_L$.

        **Pass if:** $|\text{extent} - 2 R_L| < K \cdot (2 R_L \cdot \frac{10(\omega_c \Delta t)^2}{12} + 2 R_L \cdot \epsilon_m \sqrt{N})$
...
- **Metrics & Pass Criteria:**
    ...
    2. **X-Projection Trajectory:**
        $\text{Extent} = (\max(x) - \min(x)) \cdot \Delta x$
        expected $2 R_L$.
...
- **Metrics & Pass Criteria:**
    ...
    1. **Hamiltonian Conservation:**
        ...
        **Pass if:** $\max|H(t) - H(0)| < K \cdot (|H_0| \cdot \frac{(\omega_c \Delta t)^2}{8} + |H_0| \cdot \epsilon_m \sqrt{N})$
    2. **Drift Velocity (Y):** Mean $v_y$ over the second half of the simulation, expected $v_{\text{drift}} = -\frac{E_x}{B_z}$

        **Pass if:** $|\bar{v}_y - v_{\text{drift}}| < K \cdot (|v_{\text{drift}}| \cdot \frac{(\omega_c \Delta t)^2}{8} + |v_{\text{drift}}| \cdot \epsilon_m \sqrt{N})$
...
and boundary condition implementation. The solver uses an iterative method with target relative residual $\text{rtol} \sim 10^{-6}$._
...
- **Metrics & Pass Criteria:**
    ...
    1. **Boundary Value Integrity:**
        $\epsilon_{BC} = \max(|\phi_{y=0} - (-10)|,\; |\phi_{y=L_y} - 10|)$
    ...
    2. **Field Uniformity (Interior):** Standard deviation of $E_y$ in the interior (excluding 2-cell border near Neumann/Dirichlet corners).
        ...
        **Pass if:** $\sigma(E_y) < K \cdot (|\bar{E}_y| \cdot \text{rtol} + |\bar{E}_y| \cdot \epsilon_m \sqrt{N_{\text{cells}}})$
...
- **Metrics & Pass Criteria:**
    ...
    1. **Boundary Flux Leakage:** Maximum of mean $|E_n|$ across all four boundaries.
        ...
        **Pass if:** $\max(\overline{|E_x|}_{\text{walls}},\; \overline{|E_y|}_{\text{walls}}) < K \cdot (E_{\text{scale}} \cdot \text{rtol} + E_{\text{scale}} \cdot \epsilon_m \sqrt{N} + E_{\text{scale}} / N_{\min})$
...
- **Metrics & Pass Criteria:**
    ...
    1. **Boundary Zero Check:**
        $\epsilon_{wall} = \max_{\text{all boundaries}} |\phi|$

        **Pass if:** $\epsilon_{wall} < K \cdot \epsilon_m \cdot \max(|\phi_{\text{boundary}}|, 1)$

    2. **Symmetry:**
        $\epsilon_{sym} = P_{99.9}(|\phi(x,y) - \phi(L_x-x, L_y-y)|)$
        over interior points (excluding 2-cell border and 3-cell radius around center singularity).

        **Pass if:** $\epsilon_{sym} < K \cdot (V_{\text{scale}} \cdot \text{rtol} + V_{\text{scale}} \cdot \epsilon_m \sqrt{N} + V_{\text{scale}} / N^{1.5})$
...
- **Metrics & Pass Criteria:**
    ...
    1. **Periodicity (Derivative Continuity):** Checks that the first derivative of $\phi$ is continuous across the periodic boundary (left-right wrap-around). Automatically detects whether the solver duplicates the endpoint ($\phi[0] = \phi[N_x]$) or not.
        ...
        **Pass if:** $\text{derivative jump} < K \cdot (\text{max internal curvature} + V_{\text{range}} \cdot \text{rtol} + V_{\text{range}} \cdot \epsilon_m \sqrt{N})$
...
- **Metrics & Pass Criteria:**
    ...
    1. **Analytical Fit ($1 - R^2$):** Coefficient of determination for $\phi$ vs $\ln(r)$ regression. Measures how well the numerical potential follows the expected logarithmic profile.
        ...
        **Pass if:** $1 - R^2 < K \cdot ((\Delta x / r_{\min})^2 + 100 \cdot \epsilon_m)$
...
_**Threshold framework:** All thresholds are adaptive, computed as $
	ext{threshold} = K \times (\text{physics\_err} + \text{float\_err})$
where $K=10$ is a safety factor. Physics error captures solver residual ($O(\text{rtol})$), discretization ($O(\Delta x^2)$), and CIC shape-function effects. Float error captures $O(\epsilon_m \sqrt{N})$ accumulation ($\epsilon_m \approx 2.22 \times 10^{-16}$). Grid charge is inferred via Gauss's law: $Q_{\text{grid}} = -\epsilon_0 \sum_{\text{interior}} (\nabla^2 \phi)_{i,j}\, \Delta x \, \Delta y$
...
- **Metrics & Pass Criteria:**
    ...
    1. **Integrated Charge Conservation:**
         ...
         **Pass if:** $\epsilon_Q < K \cdot |Q_{\text{total}}| \cdot (\text{rtol} + 1/N + \epsilon_m \sqrt{N_{\text{cells}}})$
        _(Combines solver residual, $O(1/N)$ Laplacian discretization error, and float accumulation.)_

    2. **Symmetry:**
        $\epsilon_{\text{sym}} = P_{99.9}(|\phi(x,y) - \phi(L_x-x, L_y-y)|)$
        over interior points (excluding 2-cell border and 3-cell radius around center singularity).

        **Pass if:** $\epsilon_{\text{sym}} < K \cdot V_{\text{scale}} \cdot (\text{rtol} + \epsilon_m \sqrt{N_{\text{pts}}} + 1/N_{\text{grid}})$
        _(Combines solver residual, float accumulation, and $O(1/N)$ grid discretization asymmetry from CIC and Laplacian.)_

    3. **L2 Profile Error:** 
        Relative RMS between simulated $\phi$ and analytic Green's function $\phi_{\text{ana}}$ (2D Fourier series for point charge in grounded box, 50 modes).

        **Pass if:** $\sqrt{\frac{\sum (\phi - \phi_{\text{ana}})^2}{\sum \phi_{\text{ana}}^2}} < K \cdot (\text{rtol} + 1/N_{\text{grid}} + \epsilon_m \sqrt{N_{\text{pts}}})$
...
- **Metrics & Pass Criteria:**
    1. **Acceleration Accuracy:** Linear fit
        $v_y(t) = a_{\text{meas}} \cdot t + v_0$
        gives measured acceleration. Expected: $a_{\text{expected}} = \frac{q E_y}{m}$
        where $E_y = -(V_{\text{top}} - V_{\text{bot}}) / L_y$.

        **Pass if:** $|a_{\text{meas}} - a_{\text{expected}}| < K \cdot |a_{\text{expected}}| \cdot (\text{rtol} + 1/N_{\text{grid}} + \epsilon_m \sqrt{N_{\text{pts}}})$
...
- **Metrics & Pass Criteria:**
    1. **Charge Conservation (Gauss):**
        ...
        **Pass if:** $\Delta Q < K \cdot |Q| \cdot (\text{rtol} + \epsilon_m \sqrt{N_{\text{cells}}} + 1/N)$
