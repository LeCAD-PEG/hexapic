 ...
 * @brief Diagnostics and output: field and particle moments, probes, and file writers.
 */
...
void create_output() {
    ...
    for (j=0; j<species.size(); j++) {
    	...
    	for (i=0; i<grid.Ncx*grid.Ncy; i++) {
    		...
    		sp.v_store[i] = std::sqrt(vx_store[i]*vx_store[i]+
    								  vy_store[i]*vy_store[i]+
...
void save_output() {
	...
	// Clear output buffer
	for (auto& vec : chunks_float) vec.clear();
	chunks_float.clear();
	...
	// save time
	MeshRecordComponent time_mesh = it.meshes["time"][MeshRecordComponent::SCALAR];
	...
	if(0) {
	    ...
	    for (i=0; i<rmpi; i++) {
	        MPI_Recv(&np_r, nsp, MPI_UNSIGNED_LONG, i, 1, 
	        			MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	        for(j=0; j<nsp; j++)  np_sum_r[j]+= np_r[j];
	    	}
