 ...
 * @brief Cartesian grid topology, ghost cells, and MPI domain decomposition utilities.
 */
...
void decompose_domain() {
	...
	if(!rmpi) {
		std::vector<std::vector<int>> rank_map(grid.Ncx, std::vector<int>(grid.Ncy, -1));	
		
		std::vector<std::vector<int>> rdom;
		rdom.resize(nmpi);
		for (i=0; i<nmpi; i++)	rdom[i].reserve(count);
		...
		for (i=0; i<nmpi; i++) {
			...
			for (y=y0; y<y0+ny; y++) {
				if(x0==0)				val = i;
				else 					val = rank_map[x0-1][y];
				rdom[i].push_back(val);
				if (!i) 	grid.neighbours.rxmin.push_back(val);
				}
			// xmax neighbours
			for (y=y0; y<y0+ny; y++) {
				if(x0+nx==grid.Ncx)	val = i;
				else 					val = rank_map[x0+nx+1][y];
				rdom[i].push_back(val);
				if (!i) 	grid.neighbours.rxmax.push_back(val);
			 	}
			// ymin neighbours 
			for (x=x0; x<x0+nx; x++) {
				if(y0==0)				val = i;
				else 					val = rank_map[x][y0-1];
				rdom[i].push_back(val);
				if (!i) 	grid.neighbours.rymin.push_back(val);
				}
			// ymax neighbours
			for (x=x0; x<x0+nx; x++) {	
				if(y0+ny==grid.Ncy)	val = i;
				else 					val = rank_map[x][y0+ny+1];
				rdom[i].push_back(val);
				if (!i) 	grid.neighbours.rymax.push_back(val);
			 	}

			// corners
			// bottom-left
			if (x0==0 || y0==0)	    		val = i;
			else							val = rank_map[x0-1][y0-1];
			rdom[i].push_back(val);
			if (!i)			grid.neighbours.corners.push_back(val);
			// top-left
			if (x0==0 || y0+ny==grid.Ncy)	val = i;
			else							val = rank_map[x0-1][y0+ny];
			rdom[i].push_back(val);
			if (!i)			grid.neighbours.corners.push_back(val);
			// bottom-right
			if (x0+nx==grid.Ncx || y0==0)	val = i;
			else							val = rank_map[x0+nx][y0-1];
			rdom[i].push_back(val);
			if (!i)			grid.neighbours.corners.push_back(val);
			if (x0+nx==grid.Ncx ||
				y0+ny==grid.Ncy)			val = i;
			else							val = rank_map[x0+nx][y0+ny];
			rdom[i].push_back(val);
			if (!i)			grid.neighbours.corners.push_back(val);
			...
			if(i)	MPI_Send(rdom[i].data(), rdom[i].size(), MPI_INT, i, 0, comm);
			}
	...
	for (int r : grid.neighbours.rxmax) grid.neighbours.all.insert(r);
	for (int r : grid.neighbours.rxmin) grid.neighbours.all.insert(r);
	for (int r : grid.neighbours.rymax) grid.neighbours.all.insert(r);
	for (int r : grid.neighbours.rymin) grid.neighbours.all.insert(r);
	for (int r : grid.neighbours.corners) grid.neighbours.all.insert(r);
	...
	for(auto& sp : species) {
		for(auto& ld : sp.load) {
			...
			if (ld.x1s > grid.x1f || ld.x2s > grid.x2f ||
				ld.x1f < grid.x1s || ld.x2f < grid.x2s)
				ld.density = 0; ///< no density
			// otherwise truncate if needed
			else {
				if (ld.x1s < grid.x1s)	ld.x1s = grid.x1s;
				if (ld.x1f > grid.x1f)	ld.x1f = grid.x1f;
				if (ld.x2s < grid.x2s)	ld.x2s = grid.x2s;
				if (ld.x2f > grid.x2f)	ld.x2f = grid.x2f;
				}
			}
		for(auto& inj : sp.inject) {
			...
			if (inj.j1 > grid.N0x+(grid.Ncx) || inj.k1 > grid.N0y+(grid.Ncy) ||
				...
				if(inj.j2-inj.j1)	inj.I /= inj.j2-inj.j1;
				if(inj.k2-inj.k1)	inj.I /= inj.k2-inj.k1;
				// truncate if needed
				if (inj.j1 < grid.N0x)				inj.j1 = grid.N0x;
				if (inj.j2 > grid.N0x+(grid.Ncx))	inj.j2 = grid.N0x+(grid.Ncx);
				if (inj.k1 < grid.N0y)				inj.k1 = grid.N0y;
				if (inj.k2 > grid.N0y+(grid.Ncy))	inj.k2 = grid.N0y+(grid.Ncy);
				}
			}
		}
	...
	grid.neighbours.max_recv_size = (2*grid.Nx+2*grid.Ny)*1000*species.size()*6;
	...
	for (int r : grid.neighbours.all) grid.neighbours.recv_data[r] = empty1;
	std::vector<std::vector<std::vector<REAL>>> empty2(species.size(), 
		std::vector<std::vector<REAL>>(nmpi));
	...
	if(print_neighbours) {
		...
		printf("\nRank %d: x0=%d, nx=%d, y0=%d, ny=%d\n", rmpi, 
			grid.N0x, grid.Ncx, grid.N0y, grid.Ncy);
		printf("rxmin:");
		...
		for (int r : grid.neighbours.all) {
			std::cout << rmpi << " to neighbour " << r << " shift_xy="
			<< grid.neighbours.shift_xy[r][0] << ", "
