 ...
 * @brief Volumetric source terms (heating/ionization profiles, source
 * sampling).
 */
...
void make_inject_basis() {
  ...
  norm = sqrt(basis[1][0] * basis[1][0] + basis[1][1] * basis[1][1] +
              basis[1][2] * basis[1][2]);
  basis[1][0] /= norm;
  basis[1][1] /= norm;
  basis[1][2] /= norm;
