 ...
 * @brief Parser for the input file and runtime configuration (geometry, species, sources, solver parameters).
 */
...
void process_section(std::string word, std::string ref) {
	...
	if (status==1 && word=="{") {
		...
		if (name=="MCC") {
			if (MCC_init_data.size()==0) MCC_init_data = {};
			MCC_init_data.push_back({});
			}
		if (name=="PSI") {
			if (PSI_init_data.size()==0) PSI_init_data = {};
	...
	if (status==2) {
		...
		if (name=="Grid") {
			if(word=="J" || word=="x1s" || word=="x1f" || 
				word=="K" || word=="x2s" || word=="x2f" ||
				word=="PeriodicFlagX1" || word=="PeriodicFlagX2") {	
				word_found=TRUE;
				subname = word;
				return;
				}
			if(word_found && word!="=") {
				...
				if (subname=="J") 			grid.Ncx = std::stoi(word);
				if (subname=="x1s")			grid.x1s = std::stof(word);
				if (subname=="x1f") 		grid.x1f = std::stof(word);
				if (subname=="K") 			grid.Ncy = std::stoi(word);
				if (subname=="x2s")			grid.x2s = std::stof(word);
				if (subname=="x2f")			grid.x2f = std::stof(word);
				if (subname=="PeriodicFlagX1") {
					int PeriodicFlagX1 = std::stoi(word);
					if (PeriodicFlagX1 == 1) {
						grid.boundaries.push_back({});
						grid.boundaries.back().add_parameters("Periodic", 0, 0,
							0, grid.Ncy);
						grid.boundaries.push_back({});
						grid.boundaries.back().add_parameters("Periodic",
							grid.Ncx, grid.Ncx, 0, grid.Ncy);
					}
				}
				if (subname=="PeriodicFlagX2") {
					...
					if (PeriodicFlagX2 == 1) {
						grid.boundaries.push_back({});
						grid.boundaries.back().add_parameters("Periodic", 0,
							grid.Ncx, 0, 0);
						grid.boundaries.push_back({});
						grid.boundaries.back().add_parameters("Periodic", 0,
							grid.Ncx, grid.Ncy, grid.Ncy);
					}
				}
		...
		if (name=="Control") {
			...
			if(word_found && word!="=") {
				//std::cout << subname << "=" << word << "\n";
				if (subname=="dt") 			dt = std::stof(word);
				if (subname=="B01")			Bf[0] = std::stof(word);
				if (subname=="B02") 		Bf[1] = std::stof(word);
				if (subname=="B03") 		Bf[2] = std::stof(word);
				word_found = FALSE;
				subname = "";
				}
			if(word=="}") {
		...
		if (name=="Load") {
			if(word=="speciesName" || word=="LoadMethodFlag" ||
				word=="x1MinMKS" || word=="x1MaxMKS" || 
				word=="x2MinMKS" || word=="x2MaxMKS" ||
				word=="density" || word=="np2c" ||
			...
			if(word_found && word!="=") {
				...
				if (subname=="x1MinMKS")	lv->x1s = std::stof(word);
				if (subname=="x1MaxMKS")	lv->x1f = std::stof(word);
				if (subname=="x2MinMKS")	lv->x2s = std::stof(word);
				if (subname=="x2MaxMKS")	lv->x2f = std::stof(word);
				if (subname=="density")		lv->density = std::stof(word);
				if (subname=="np2c")		
					lv->np2c = (int)std::stof(word);
				if (subname=="v1thermal")	lv->v1thermal = std::stof(word);
				if (subname=="v2thermal")	lv->v2thermal = std::stof(word);
				if (subname=="v3thermal")	lv->v3thermal = std::stof(word);
				if (subname=="v1drift")		lv->v1drift = std::stof(word);
				if (subname=="v2drift")		lv->v2drift = std::stof(word);
				if (subname=="v3drift")		lv->v3drift = std::stof(word);
				if (subname=="LoadMethodFlag")	lv->method = std::stoi(word);
				word_found = FALSE;
				subname = "";
				}
			if(word=="}") {
		...
		if (name=="EmitPort") {
			if(word=="speciesName" || word=="I" || 
				word=="j1" || word=="j2" || 
				word=="k1" || word=="k2" || word=="np2c" || word=="normal" ||
				word=="v1thermal" || word=="v2thermal" || word=="v3thermal" ||
				word=="v1drift" || word=="v2drift" || word=="v3drift") {	
				word_found=TRUE;
				subname = word;
			...
			if(word_found && word!="=") {
				...
				if (subname=="I")	iv->I = fabs(std::stof(word));
				if (subname=="j1")	iv->j1 = std::stoi(word);
				if (subname=="j2")	iv->j2 = std::stoi(word);
				if (subname=="k1")	iv->k1 = std::stoi(word);
				if (subname=="k2")	iv->k2 = std::stoi(word);
				if (subname=="normal")	iv->normal = std::stoi(word);
				if (subname=="np2c")		
					iv->np2c = (int)std::stof(word);
				if (subname=="v1thermal")	iv->v1thermal = std::stof(word);
				if (subname=="v2thermal")	iv->v2thermal = std::stof(word);
				if (subname=="v3thermal")	iv->v3thermal = std::stof(word);
				if (subname=="v1drift")		iv->v1drift = std::stof(word);
				if (subname=="v2drift")		iv->v2drift = std::stof(word);
				if (subname=="v3drift")		iv->v3drift = std::stof(word);
				word_found = FALSE;
				subname = "";
				}
			if(word=="}") {
		...
		if (name=="MCC") {
			if(word=="collName" || word=="specie1Name" || word=="specie2Name" ||
				word=="reactionIdx" || word=="collXSectionFile") {	
				word_found=TRUE;
				subname = word;
				return;
				}
			if(word_found && word!="=") {
				//std::cout << subname << "=" << word << "\n";
				if (subname=="collName" || subname=="specie1Name" || subname=="specie2Name" ||
					subname=="reactionIdx" || subname=="collXSectionFile")		
		...
		if (name=="PSI") {
			if(word=="boundary" || word=="specie" || word=="secondary1" ||
				word=="secondary2" || word=="secondary3" ||
				word=="sec_type" || word=="inj_type" || word=="E0" ||
				word=="Ew" || word=="gamma0" || word=="T0" || word=="Ep") {	
				word_found=TRUE;
				subname = word;
				return;
				}
...
void read_input_file(char **argv) {
    ...
    while (file >> word)  {
    	if(Description<3) {
	    	if(!Description)	{
	    		if(word=="{")	Description=2;
	    		else   			title.append(word);
	    		continue; 
	    		}
	    	if(Description)	{
	    		if(word=="}")	Description=3;
	    		else {
    ...
    if(!rmpi) {
	    // description
		std::cout << "INPUT: " << "title = " << title << "\n";
		std::cout << "INPUT: " << "description =" << description << "\n";
