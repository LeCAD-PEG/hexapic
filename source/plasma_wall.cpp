 ...
 * @brief Boundary conditions and plasma–wall interaction (sheath models, secondary emission, reflection).
 */
...
void secondary_emission(REAL& E, REAL sigmd, int sp, int sec_i,
						int dir, REAL xy_along, REAL z);
...
void inject_secondary(int sp2, int inj_type, REAL& E, REAL T0,
					  int dir, REAL xy_along, REAL z);
...
void PSI_init() {
	...
	for (auto& bc : grid.boundaries) {
		bc.sp_sec = {};
		for (int i=0; i<species.size(); i++) bc.sp_sec.push_back({});
	}
	...
	for (int i=0; i<PSI_init_data.size(); i++) {
		...
		se_e.sp2 = {};
		for (int j=0; j<n-9; j++) se_e.sp2.push_back(specie_index(psi[2+j]));
...
void particle_wall_interaction(size_t cell_i, int sp, int p_i, int lrbt) {
	...
	*/
	...
	if (lrbt < 4) { // just one interaction
		if (lrbt==0)      wall_interaction(cell_i, sp, p_i, 0);
		else if (lrbt==1) wall_interaction(cell_i, sp, p_i, 1);
		else if (lrbt==2) wall_interaction(cell_i, sp, p_i, 2);
		else              wall_interaction(cell_i, sp, p_i, 3);
	}
...
void wall_interaction(size_t cell_i, int sp, int p_i, int dir) {
	...
	if (species[sp].charge == 0) { // Neutrals get reflected except if open bc
		...
		if( (dir<2  && p.y>=0 && p.y<grid.Ncy) ||
			...
			std::cout<<"Warning: PSI particle outside of grid: x="<<p.x<<", y="<<p.y<<
			", Nx="<<grid.Ncx<<", Ny="<<grid.Ncy<<", dir="<<dir<<std::endl;
		}
	}
}
...
void reflect_particle(size_t cell_i, int sp, int p_i, int dir) {
	...
	if (dir < 2) {
		p.vx = -p.vx;
		if (dir == 0) p.x = -p.x;
		else p.x = 2*grid.Ncx-p.x;
	}
	else {
		p.vy = -p.vy;
		if (dir == 2) p.y = -p.y;
		else p.y = 2*grid.Ncy-p.y;
	}
...
void sigmd_E(REAL& sigmd, REAL& E, size_t cell_i, int sp, int p_i, int dir) {
	...
	if (dir<2) sigmd = sqrt(v_sq)/fabs(p.vx); ///< x direction
	else sigmd = sqrt(v_sq)/fabs(p.vy); ///< y direction
}
...
void secondary_emission(REAL& E, REAL sigmd, int sp, int sec_i,
	...
	if (SE.sec_type == 0) { // electron-induced SEE
    		...
    		// Optional physical cap: max secondaries limited by available energy scale Ew
    		int Ncap = INT_MAX;
    		if (SE.Ew > 0) {
		...
		REAL frac  = gamma_sec - (REAL)n_emit;
    		if (frand() < frac) ++n_emit;
    		if (n_emit > Ncap) n_emit = Ncap;
    		if (n_emit <= 0) return;

    		// Secondary energy: reuse Ew (~few eV). If you later add a sampler, seed with T0.
		 ...
		 // Expected yield
    		REAL delta = gamma_i(E, sigmd, SE.gamma0, SE.E0, SE.Ep, int(species[1].mass/1e-27));
		...
		if (gamma_sec > frand()) { // H injection
			...
			inject_secondary(SE.sp2[0], 1, E, 0.5,
				dir, xy_along, z);
	...
	else std::cout << "Secondary emission of type " << SE.sec_type
		<< " not yet implemented!\n";
...
REAL gamma_e(REAL E, REAL sigmd, REAL gamma0, REAL E0, REAL Ep) {
    if (E <= 0 || gamma0 <= 0) return 0;

    // Map & sanitize inputs
    REAL delta_max = gamma0;                       // peak yield
    REAL Emax      = (E0 > 50.0) ? E0 : 350.0;     // tungsten-ish default if E0 looks wrong
...
void inject_secondary(int sp2, int inj_type, REAL& E, REAL T0,
	...
	//std::cout<<"DEBUG: sp2="<<sp2<<", inj_type="<<inj_type<<", dir="<<dir<<", p.x="<<p.x<<", p.y="<<p.y<<", cell_i="<<cell_i<<std::endl;
