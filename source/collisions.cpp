 ...
 * 
 * @file collisions.cpp
 * @brief Monte-Carlo Collisions (MCC): elastic, ionization, excitation, and charge-exchange processes.
 */
...
void COM_velocity(REAL v_com[3], int cell_i,
	int sp_1, int sp_2, int p_1, int p_2);
void isotropic_scatter(REAL v_scatt[3]);
void isotropic_scatter(REAL v_scatt[3], REAL v);
void null_collision(int coll_i, int react_i, int cell_i,
	int sp_1, int sp_2, int p_1, int p_2, REAL E);
void elastic_scattering_collision(int coll_i, int react_i, int cell_i,
	int sp_1, int sp_2, int p_1, int p_2, REAL E);
void excitation_collision(int coll_i, int react_i, int cell_i,
	int sp_1, int sp_2, int p_1, int p_2, REAL E);
void ionisation_collision(int coll_i, int react_i, int cell_i,
	int sp_1, int sp_2, int p_1, int p_2, REAL E);
void charge_exchange_collision(int coll_i, int react_i, int cell_i,
	int sp_1, int sp_2, int p_1, int p_2, REAL E);
...
void collisions_init() {
	collisions = {};
	for (int i=0; i<MCC_init_data.size(); i++) read_add_collision(i);
	for (int i=0; i<collisions.size(); i++) set_maximal_sigma_v(i);
...
void read_add_collision(int i) {
	...
	if (species[sp1].mass > species[sp2].mass) std::swap(sp1, sp2);
	...
	if (new_collision) add_new_empty_collision(sp1, sp2);
...
int specie_index(std::string specie_name) {
	for (int i=0; i<species.size(); i++) {
		if (species[i].name == specie_name) return i;
	}
...
void test_sp_reaction_compatibility(int sp1, int sp2, int reaction_index) {
	...
	if (error) std::cout << "Species " << species[sp1].name << ", "
		<< species[sp2].name << " not compatible with reaction "
		<< reaction_index << "\n";
}
...
reactionFunction get_reaction(int reaction_index) {
	...
	if (reaction_index==0) return &null_collision;
	else if (reaction_index==1) return &elastic_scattering_collision;
	else if (reaction_index==2) return &excitation_collision;
	else if (reaction_index==3) return &ionisation_collision;
	else if (reaction_index==5) return &charge_exchange_collision;
	else {
...
void add_sp_out(int coll_i, int reaction_index) {
	...
	if (reaction_index==0 || reaction_index==1 || reaction_index==2)
		...
		if (sp2 == collisions[coll_i].sp[1] && sp2 == species.size()) {
			std::cout << "ERROR: Proper ion specie of "
			<< species[sp2].name << " not specified!\n";
...
void set_maximal_sigma_v(int coll_i) {
	...
	for (int j=0; j<coll.n_reactions; j++)
		for (auto E_sigma : coll.sigma[j]) E_all.insert(E_sigma.first);
...
REAL read_cross_section(int coll_i, int j, REAL E) {
	...
	if (it_next == sigma.end()) return 0.0;
	else if (it_next == sigma.begin()) return 0.0;
	else {
	...
	for (i=0; i<N_coll; i++) {
		...
		for (j=0; j<coll.n_reactions; j++) {
			...
			if (nu_cum > R) { // collision j actually takes place
				...
				coll.processCollision[j](coll_i, j, cell_i, sp1, sp2,
					p1, p2, E);
				break;
			}
		}
...
void recombination_collision(int coll_i, int react_i, int cell_i,
	int sp_1, int sp_2, int p_1, int p_2, REAL E) {
	// reaction_index = 4
	// e + (A+) → A + γ
